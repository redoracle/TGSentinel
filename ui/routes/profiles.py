...
@profiles_bp.get("/alert/<profile_id>")
def get_alert_profile_by_id(profile_id):
    """Get alert profile by ID (REST-style endpoint for compatibility)."""
    try:
        profile_id_int = int(profile_id)
        profile = (
            _profile_service.get_alert_profile(profile_id_int)
            if _profile_service
            else None
        )
        if not profile:
...
# ═══════════════════════════════════════════════════════════════════
# Feedback Stats Proxy Routes (Proxy to Sentinel)
# ═══════════════════════════════════════════════════════════════════


@profiles_bp.get("/alert/<int:profile_id>/feedback-stats")
def get_alert_feedback_stats(profile_id: int):
    """Proxy alert profile feedback stats from Sentinel.

    Args:
        profile_id: Alert profile ID (1000-1999)

    Returns:
        JSON with feedback statistics from Sentinel.
    """
    try:
        sentinel_url = os.getenv("SENTINEL_API_BASE_URL", "http://sentinel:8080/api")
        url = f"{sentinel_url}/profiles/alert/{profile_id}/feedback-stats"

        logger.debug(f"Proxying alert feedback stats request to: {url}")
        ...
        logger.error(
            f"Failed to fetch alert feedback stats for profile {profile_id}: {exc}",
            exc_info=True,
        )
        return (
            jsonify(
                {
                    "status": "error",
                    "message": f"Failed to fetch feedback stats: {str(exc)}",
                }
            ),
            500,
        )
        ...
        logger.error(
            f"Unexpected error fetching alert feedback stats for profile {profile_id}: {exc}",
            exc_info=True,
        )
        return jsonify({"status": "error", "message": str(exc)}), 500


@profiles_bp.get("/interest/<int:profile_id>/feedback-stats")
def get_interest_feedback_stats(profile_id: int):
    """Proxy interest profile feedback stats from Sentinel.
    ...
    try:
        sentinel_url = os.getenv("SENTINEL_API_BASE_URL", "http://sentinel:8080/api")
        url = f"{sentinel_url}/profiles/interest/{profile_id}/feedback-stats"

        logger.debug(f"Proxying interest feedback stats request to: {url}")
        ...
        logger.error(
            f"Failed to fetch interest feedback stats for profile {profile_id}: {exc}",
            exc_info=True,
        )
        return (
            jsonify(
        ...
        logger.error(
            f"Unexpected error fetching interest feedback stats for profile {profile_id}: {exc}",
            exc_info=True,
        )
        return jsonify({"status": "error", "message": str(exc)}), 500
