...
# =============================================================================
# Sentinel API Proxy Routes
# These routes use data_service to fetch from Sentinel and transform data.
# The UI does not access sentinel.db directly - data_service proxies via HTTP.
# =============================================================================


@dashboard_bp.route("/alerts", methods=["GET"])
def proxy_alerts():
    """Get alerts via data_service (which fetches from Sentinel API).

    Uses data_service.load_alerts() which already handles Sentinel API
    communication and data transformation for the UI.
    Returns the response envelope: {status: 'ok', data: {alerts: [...]}}
    """
    deps = get_deps()
    limit = min(int(request.args.get("limit", 100)), 250)
    try:
        alerts = deps.data_service.load_alerts(limit=limit) if deps.data_service else []
        return jsonify(
            {
                "status": "ok",
                "data": {"alerts": alerts},
...
def proxy_interests():
    """Get interests via data_service (which fetches from Sentinel API).

    Uses data_service.load_interests() which already handles Sentinel API
    communication and data transformation for the UI.
    Returns the response envelope: {status: 'ok', data: {interests: [...]}}
    """
    deps = get_deps()
    limit = min(int(request.args.get("limit", 100)), 250)
    try:
        interests = (
            deps.data_service.load_interests(limit=limit) if deps.data_service else []
        )
        return jsonify(
            {
                "status": "ok",
...
@dashboard_bp.route("/digests", methods=["GET"])
def proxy_digests():
    """Get digests via data_service (which fetches from Sentinel API).

    Uses data_service.load_digests() which already handles Sentinel API
    communication and data transformation for the UI.
    Returns the response envelope: {status: 'ok', data: {digests: [...]}}
    """
    deps = get_deps()
    limit = min(int(request.args.get("limit", 14)), 100)
    try:
        digests = (
            deps.data_service.load_digests(limit=limit) if deps.data_service else []
        )
        ...
        logger.error(f"Failed to load digests: {e}")
        return jsonify({"status": "error", "data": None, "error": str(e)}), 500
