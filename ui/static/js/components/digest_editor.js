...
/**
 * Utility object for legacy code that expects window.DigestEditor
 * Provides static methods for extracting/populating digest config from forms
 */
window.DigestEditor = {
    /**
     * Extract digest configuration from form fields
     * @param {string} prefix - Form field prefix (e.g., 'alert-' or 'interest-')
     * @returns {Object} Digest configuration object or {error: string} on validation failure
     */
    extractDigestConfigFromForm: function(prefix) {
        ...
        // Extract schedules from schedule rows
        const schedules = [];
        const scheduleTypes = [];
        const rows = document.querySelectorAll(`#${prefix}digest-schedule-container .digest-schedule-row`);
        
        rows.forEach(row => {
            const scheduleType = row.querySelector('.digest-schedule-type')?.value;
            if (!scheduleType) return;
            
            // Track schedule types for duplicate detection
        ...
        // Validate no duplicate schedule types
        const uniqueScheduleTypes = new Set(scheduleTypes);
        if (scheduleTypes.length !== uniqueScheduleTypes.size) {
            const duplicates = scheduleTypes.filter((item, index) => scheduleTypes.indexOf(item) !== index);
            return {
                error: `Duplicate schedule types not allowed: ${[...new Set(duplicates)].join(', ')}. Each schedule type can only appear once.`
            };
        }
    ...
    /**
     * Populate form fields with digest configuration
     * @param {Object} digestConfig - Digest configuration object or null
     * @param {string} prefix - Form field prefix (e.g., 'alert-' or 'interest-')
     */
    populateDigestConfigInForm: function(digestConfig, prefix) {
        if (!digestConfig) {
            // Reset to defaults
            const modeField = document.getElementById(`${prefix}digest-mode`);
            const channelField = document.getElementById(`${prefix}digest-target-channel`);
            
            if (modeField) modeField.value = 'dm';
            if (channelField) channelField.value = '';
            
            // Clear schedule container
            const container = document.querySelector(`#${prefix}digest-schedule-container`);
            if (container) container.innerHTML = '';
            
            return;
        }
        
        // Populate mode and target channel
        const modeField = document.getElementById(`${prefix}digest-mode`);
        const channelField = document.getElementById(`${prefix}digest-target-channel`);
        
        if (modeField) modeField.value = digestConfig.mode || 'dm';
        if (channelField) channelField.value = digestConfig.target_channel || '';
        
        // Note: Schedule population is typically handled by profile-specific code
        // that knows how to render schedule rows. This method just sets the basic fields.
    }
};
