...
(function() {
     ...
     * 
     * Schedule semantics:
     * - schedule_type: "none" = instant alerts mode (no batching)
     * - schedule_type: "hourly"|"daily"|etc = batched digest mode
     * - mode: "none" = save only, "dm" = instant DM, "digest" = digest only, "both" = dm + digest
     * 
     * Schedule 1 is ALWAYS saved (even with schedule_type="none") to preserve delivery mode.
    ...
    function populateInterestDigestSchedules(schedules) {
        ...
        schedules.forEach((schedule, index) => {
            if (index >= 3) return;
            const i = index + 1;
            // Support both 'schedule_type' (UI sends) and 'schedule' (backend normalizes to)
