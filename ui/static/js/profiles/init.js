...
(function() {
    ...
    function init() {
        ...
        // Handle URL hash for deep linking to profiles (e.g., #alert-1001, #interest-3001)
        handleProfileDeepLink();
    }
    
    /**
     * Handle URL hash fragments to deep link to specific profiles.
     * Supports formats: #alert-{id} and #interest-{id}
     * This is used when clicking profile badges in the feeds page.
     */
    function handleProfileDeepLink() {
        const hash = window.location.hash;
        if (!hash) return;
        
        // Parse hash: #alert-1001 or #interest-3001
        const alertMatch = hash.match(/^#alert-(\d+)$/);
        const interestMatch = hash.match(/^#interest-(\d+)$/);
        
        if (alertMatch) {
            const profileId = alertMatch[1];
            console.log(`[PROFILES] Deep linking to alert profile: ${profileId}`);
            
            // Switch to alert profiles tab
            const alertTab = document.getElementById('alert-profiles-tab');
            if (alertTab) {
                const tab = new bootstrap.Tab(alertTab);
                tab.show();
            }
            
            // Wait for profiles to load, then select the profile
            setTimeout(() => {
                if (window.AlertProfiles && window.AlertProfiles.loadAlertProfile) {
                    window.AlertProfiles.loadAlertProfile(profileId);
                    // Scroll to form
            ...
            const profileId = interestMatch[1];
            console.log(`[PROFILES] Deep linking to interest profile: ${profileId}`);
            
            // Switch to interest profiles tab
            const interestTab = document.getElementById('interest-profiles-tab');
            if (interestTab) {
                const tab = new bootstrap.Tab(interestTab);
                tab.show();
            }
            
            // Wait for profiles to load, then select the profile
            setTimeout(() => {
                if (window.InterestProfiles && window.InterestProfiles.selectInterestProfile) {
                    window.InterestProfiles.selectInterestProfile(profileId);
                    // Scroll to form
        ...
        // Clear the hash after handling to avoid re-triggering on refresh
        // Use replaceState to avoid adding a history entry
        if (alertMatch || interestMatch) {
