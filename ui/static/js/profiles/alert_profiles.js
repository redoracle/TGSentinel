...
(function() {
     ...
     */
    function toggleAlertScheduleFields(scheduleNum, scheduleType) {
        const dailySettings = document.getElementById(`alert-digest-schedule-${scheduleNum}-daily-settings`);
        const weeklySettings = document.getElementById(`alert-digest-schedule-${scheduleNum}-weekly-settings`);
        const weeklyHourSettings = document.getElementById(`alert-digest-schedule-${scheduleNum}-weekly-hour-settings`);
     ...
     */
    function toggleAlertDigestBatchSettings(scheduleNum, deliveryMode) {
        const topNWrapper = document.getElementById(`alert-digest-schedule-${scheduleNum}-topn-wrapper`);
        const minScoreWrapper = document.getElementById(`alert-digest-schedule-${scheduleNum}-minscore-wrapper`);
    ...
    function attachScheduleTypeListeners() {
        for (let i = 1; i <= 3; i++) {
            const typeSelect = document.getElementById(`alert-digest-schedule-${i}-type`);
            const modeSelect = document.getElementById(`alert-digest-schedule-${i}-mode`);
            
            if (typeSelect) {
                // Capture the current value of i
                const scheduleNum = i;
                typeSelect.addEventListener('change', function() {
                    toggleAlertScheduleFields(scheduleNum, this.value);
                });
                // Initialize visibility on page load
                toggleAlertScheduleFields(scheduleNum, typeSelect.value);
            }
            
            if (modeSelect) {
                const scheduleNum = i;
                modeSelect.addEventListener('change', function() {
                    toggleAlertDigestBatchSettings(scheduleNum, this.value);
                });
                // Initialize visibility on page load
                toggleAlertDigestBatchSettings(scheduleNum, modeSelect.value);
            }
        }
    }
    ...
    async function loadAlertProfile(profileId) {
        try {
            ...
            document.getElementById("alert-prioritize-private").checked = data.profile.prioritize_private || false;
            
            // Engagement thresholds
            document.getElementById("alert-reaction-threshold").value = data.profile.reaction_threshold || 0;
            document.getElementById("alert-reply-threshold").value = data.profile.reply_threshold || 0;
            ...
            const digest = data.profile.digest || data.profile.digest_config || null;
            const defaultChannel = document.querySelector('[data-default-channel]')?.dataset.defaultChannel || '';
            
            if (digest && digest.schedules) {
                // Clear all schedules first and hide schedule-specific fields
                for (let i = 1; i <= 3; i++) {
                    document.getElementById(`alert-digest-schedule-${i}-type`).value = "";
                    document.getElementById(`alert-digest-schedule-${i}-top-n`).value = 10 * i;
                    document.getElementById(`alert-digest-schedule-${i}-min-score`).value = 5.0 - i;
                    document.getElementById(`alert-digest-schedule-${i}-mode`).value = "dm";
                    document.getElementById(`alert-digest-schedule-${i}-target-channel`).value = defaultChannel;
                    toggleAlertScheduleFields(i, '');  // Hide all schedule-specific fields
                    toggleAlertDigestBatchSettings(i, 'dm');  // Hide batch settings for DM mode
                }
                
                // Populate from digest.schedules
                digest.schedules.forEach((schedule, index) => {
                    if (index >= 3) return;
                    const i = index + 1;
                    // Handle both field names (schedule_type from UI, schedule from backend normalization)
                    // Map "none" back to empty string for UI (HTML select uses "" for "None")
                    let scheduleType = schedule.schedule_type || schedule.schedule || "";
                    if (scheduleType === "none") scheduleType = "";
                    document.getElementById(`alert-digest-schedule-${i}-type`).value = scheduleType;
                    document.getElementById(`alert-digest-schedule-${i}-top-n`).value = schedule.top_n || 10;
                    document.getElementById(`alert-digest-schedule-${i}-min-score`).value = schedule.min_score || 5.0;
                    
                    // Per-schedule mode and target_channel (with fallback to profile-level for backward compat)
                    const schedMode = schedule.mode || digest.mode || "dm";
                    const schedChannel = schedule.target_channel || digest.target_channel || defaultChannel;
                    document.getElementById(`alert-digest-schedule-${i}-mode`).value = schedMode;
                    document.getElementById(`alert-digest-schedule-${i}-target-channel`).value = schedChannel;
                    
                    // Set schedule-specific fields
                    if (scheduleType === 'daily') {
                        document.getElementById(`alert-digest-schedule-${i}-daily-hour`).value = schedule.daily_hour || 8;
                    } else if (scheduleType === 'weekly') {
                        document.getElementById(`alert-digest-schedule-${i}-weekly-day`).value = schedule.weekly_day || 0;
                        document.getElementById(`alert-digest-schedule-${i}-weekly-hour`).value = schedule.weekly_hour || 8;
                    }
                    
                    // Toggle visibility of schedule-specific fields
                    toggleAlertScheduleFields(i, scheduleType);
                    // Toggle visibility of batch settings based on delivery mode
                    toggleAlertDigestBatchSettings(i, schedMode);
                });
            }
    ...
    async function saveAlertProfile(event) {
        ...
        const profileData = {
            ...
            prioritize_private: document.getElementById("alert-prioritize-private").checked,
            // Engagement thresholds
            reaction_threshold: parseInt(document.getElementById("alert-reaction-threshold").value) || 0,
            reply_threshold: parseInt(document.getElementById("alert-reply-threshold").value) || 0
        };
        ...
        // Extract digest configuration from schedule forms
        // ALWAYS save Schedule 1 (even with empty type as "none") so Delivery Mode persists
        // Schedules 2-3 are optional and only saved if Schedule Type is set
        const digestSchedules = [];
        const defaultChannel = document.querySelector('[data-default-channel]')?.dataset.defaultChannel || '';
        
        for (let i = 1; i <= 3; i++) {
            const type = document.getElementById(`alert-digest-schedule-${i}-type`)?.value;
            
            // Schedule 1: always save (use "none" for empty type to preserve delivery mode)
            // Schedules 2-3: only save if type is set
            if (i > 1 && !type) continue;
            
            const schedule = {
                schedule_type: type || "none",  // Use "none" for empty type
                top_n: parseInt(document.getElementById(`alert-digest-schedule-${i}-top-n`)?.value) || 10,
                min_score: parseFloat(document.getElementById(`alert-digest-schedule-${i}-min-score`)?.value) || 5.0,
                mode: document.getElementById(`alert-digest-schedule-${i}-mode`)?.value || 'dm',
                target_channel: document.getElementById(`alert-digest-schedule-${i}-target-channel`)?.value?.trim() || null
            };
            
            if (type === 'daily') {
                schedule.daily_hour = parseInt(document.getElementById(`alert-digest-schedule-${i}-daily-hour`)?.value) || 8;
            } else if (type === 'weekly') {
                schedule.weekly_day = parseInt(document.getElementById(`alert-digest-schedule-${i}-weekly-day`)?.value) || 0;
                schedule.weekly_hour = parseInt(document.getElementById(`alert-digest-schedule-${i}-weekly-hour`)?.value) || 8;
            }
            
            digestSchedules.push(schedule);
        }
        
        // Always include digest config if we have schedules (Schedule 1 is always saved)
        if (digestSchedules.length > 0) {
            profileData.digest = {
                schedules: digestSchedules
            };
        }
            ...
            if (data.matches && data.matches.length > 0) {
                tbody.innerHTML = data.matches.map(match => {
                    ...
                    const webhookPayload = {
                        event: "alert_backtest",
                        profile_id: data.profile_id || profileId,
                        profile_name: data.profile_name || document.getElementById("alert-name")?.value || "",
                ...
                const webhookColspan = colspan + 1;
                tbody.innerHTML = `<tr><td colspan="${webhookColspan}" class="text-center text-muted">No matches found</td></tr>`;
            }
    ...
    function resetAlertProfileForm() {
        ...
        // Clear digest configuration and hide schedule-specific fields
        if (window.DigestEditor) {
            window.DigestEditor.populateDigestConfigInForm(null, 'alert-');
        }
        
        // Hide all schedule-specific fields and batch settings after reset
        for (let i = 1; i <= 3; i++) {
            toggleAlertScheduleFields(i, '');
            toggleAlertDigestBatchSettings(i, 'dm');  // Default to DM mode (hide batch settings)
        }
        
        // Reset Delivery Settings to defaults
        document.getElementById("alert-digest-mode").value = "dm";
        document.getElementById("alert-digest-target-channel").value = "";
    ...
    function updateMaxScoreDisplay() {
        ...
        const detectionToggles = [
            { id: 'alert-detect-questions', score: 1.2, name: 'Questions' },  // Max 1.2 (private chats only)
            ...
            { id: 'alert-prioritize-private', score: 0.5, name: 'Private' }
        ];
        ...
        // Engagement thresholds (+0.5 each when configured > 0)
        const reactionThreshold = parseInt(document.getElementById('alert-reaction-threshold')?.value) || 0;
        const replyThreshold = parseInt(document.getElementById('alert-reply-threshold')?.value) || 0;
    ...
    function attachMaxScoreListeners() {
        ...
        // Listen to engagement threshold changes
        const thresholdIds = ['alert-reaction-threshold', 'alert-reply-threshold'];
        thresholdIds.forEach(thresholdId => {
