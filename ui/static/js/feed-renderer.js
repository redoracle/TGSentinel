...
function renderFeedItem(item, feedType = 'alert') {
    // Check if this is a placeholder message (no real data yet)
    if (item.chat_id === -1) {
        return `
            <div class="list-group-item alert-list-item">
                <div class="d-flex w-100 justify-content-between align-items-start gap-3">
                    <div class="flex-grow-1 min-w-0">
                        <!-- Channel Name (no link for placeholder) -->
...
function renderEmptyState(feedType = 'alert') {
    const messages = {
        alert: {
            title: 'No Alerts Yet',
            subtitle: 'Alerts will appear here once heuristics fire.'
        },
        interest: {
