...
const manualDigestEndpoint = "/sentinel/digests/trigger";
...
async function refreshAlerts() {
    try {
        const response = await fetch(`${alertsEndpoint}?limit=50`);
        if (!response.ok) {
...
async function refreshInterests() {
    try {
        const response = await fetch(`${interestsEndpoint}?limit=50`);
        if (!response.ok) {
...
async function triggerManualDigest(type) {
    const endpoint = manualDigestEndpoint;
    const buttonId =
        type === "interests" ? "btn-trigger-interests-digest" : "btn-trigger-alerts-digest";
    ...
    try {
        ...
        if (!response.ok || result.status !== "ok") {
            throw new Error(
                result.message || result.error?.message || "Digest trigger failed"
            );
        }

        showToast(`Manual ${type} digest triggered`, "success");
    } catch (error) {
        console.error("Digest trigger error:", error);
        showToast(`Failed to trigger ${type} digest`, "error");
    } finally {
        if (button) {
...
function bindInteractions() {
    ...
    document.getElementById("btn-trigger-alerts-digest")?.addEventListener("click", () =>
        triggerManualDigest("alerts")
    );
    document.getElementById("btn-trigger-interests-digest")?.addEventListener("click", () =>
        triggerManualDigest("interests")
    );
