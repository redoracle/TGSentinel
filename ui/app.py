...
# Compatibility wrapper — prefer validators module, fallback to shim
try:
...
# Get the directory where this file is located for proper static/template paths
...
# Add CSP headers - fully relaxed for private internal app
@app.after_request
def add_security_headers(response):
    # Fully relaxed CSP for private internal app
    # Monaco Editor requires: eval, blob workers, CDN resources
    # Socket.IO requires: websockets, CDN
...
def init_app() -> None:
    ...
    with _init_lock:
        ...
        stream_cfg = (
            {
                ...
                "stream": config.system.redis.stream,
            }
            if config
            else {
        ...
        if config:
            logger.info(
                f"Dependency container populated with config: "
                f"channels={len(config.channels) if hasattr(config, 'channels') else 'N/A'}, "
                f"users={len(config.monitored_users) if hasattr(config, 'monitored_users') else 'N/A'}"
            )
        else:
        ...
        def check_authentication():
            ...
            if path.startswith("/api/"):
                ...
                # Check UI lock for other APIs (but allow feedback-stats for authenticated users)
                if is_locked and not path.endswith("/feedback-stats"):
                    return (
...
def _query_one(sql: str, **params: Any) -> Any:
    """Legacy stub: UI database removed; use Sentinel API instead."""
    logger.warning("DEPRECATED: _query_one() called but UI DB is removed.")
    return None


def _query_all(sql: str, **params: Any) -> List[Dict[str, Any]]:
    """Legacy stub: UI database removed; use Sentinel API instead."""
    logger.warning("DEPRECATED: _query_all() called but UI DB is removed.")
    return []


def _execute(sql: str, **params: Any) -> None:
    """Legacy stub: UI database removed; use Sentinel API instead."""
    logger.warning("DEPRECATED: _execute() called but UI DB is removed.")
    return
