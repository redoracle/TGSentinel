...
def serve_data_file(filename):
    ...
    SECURITY: Session files (.session*) are never served via this route.
    Use /api/session/download endpoint for session backups.
    """
    # Block any .session file access
    if filename.endswith(".session") or ".session" in filename:
        logger.warning(
            f"[SECURITY] Blocked attempt to access session file via /data: {filename}"
        )
        return "Forbidden: Session files cannot be accessed directly", 403
