    ...
    if "alerts" in config_data:
        ...
        if "mode" in alerts:
            ...
            if mode not in ("dm", "channel", "both"):
                return (
                    False,
                    f"alerts.mode must be 'dm', 'channel', or 'both', got: {mode}",
                )
    ...
    try:
        ...
        for msg in messages:
            ...
            for item in compiled_rules:
                ...
                if len(text) > 10000:
                    logger.warning(
                        f"Skipping regex on text longer than 10000 chars to prevent ReDoS"
                    )
                    continue
...
def api_config_channels_add():
    """Add new channels to monitoring configuration."""
    try:
        data = request.get_json()
