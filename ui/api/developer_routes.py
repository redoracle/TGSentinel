...
def api_webhooks_create():
    ...
    try:
        webhooks_path = _resolve_config_path("webhooks.yml")
        _ensure_parent_dir(webhooks_path)
        ...
        if secret:
            ...
            new_webhook["secret"] = encrypt_secret(secret)
        ...
        # Save atomically
        temp_dir = (
            webhooks_path.parent
            if isinstance(webhooks_path, Path)
            else getattr(webhooks_path, "parent", None)
        )
        ...
        _publish_integration_event("webhook_created", {"service": service, "url": url})
    ...
    try:
        ...
        if secret:
            ...
            try:
                ...
                logger.error(
                    f"Failed to decrypt webhook secret for {service_name}: {exc}"
                )
                return (
                    jsonify(
                        {
                            "status": "error",
                            "message": f"Failed to decrypt webhook secret: {str(exc)}",
                        }
                    ),
                    500,
                )
        ...
        try:
            ...
            return (
                jsonify(
                    {
                        "status": "error",
                        "message": f"Webhook request timed out after 10 seconds",
                    }
                ),
                504,
            )
...
def api_webhooks_update(service_name: str):
    ...
    try:
        ...
        webhook = None
        for wh in webhooks:
            if wh.get("service") == service_name:
                webhook = wh
                break
        ...
        if "secret" in payload:
            ...
            if secret:
                webhook["secret"] = encrypt_secret(secret)
            else:
