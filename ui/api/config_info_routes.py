...
def _fetch_sentinel_config() -> tuple[dict, Optional[str]]:
    ...
    """
    try:
        response = http_requests.get(f"{_get_sentinel_api_url()}/config", timeout=5)
        if not response.ok:
...
def _update_sentinel_config(config_updates: dict) -> tuple[bool, Optional[str]]:
    ...
    try:
        response = http_requests.post(
            f"{_get_sentinel_api_url()}/config",
            json=config_updates,
...
def _refresh_ui_config(context: str) -> None:
    ...
    Note: This is a transitional function. As UI moves to fully proxying to Sentinel,
    this refresh becomes less critical since Sentinel is source of truth.
    """
    try:
        from ui.core import get_deps

        # Config is now refreshed via Sentinel API call, not local file
        deps = get_deps()
        if hasattr(deps, "reload_config"):
            deps.reload_config()  # type: ignore[attr-defined]
        logger.info("Signaled config reload after %s", context)
    except Exception as exc:  # pragma: no cover - defensive path
        logger.warning("Failed to signal config reload after %s: %s", context, exc)
...
def api_config_current():
    ...
    try:
        ...
        # Phone number comes from cached_user (authenticated session) only
        ...
        logger.info(
            f"Fetched config from Sentinel: channels={len(config_data.get('channels', []))}, "
            f"users={len(config_data.get('monitored_users', []))}"
        )
...
@config_info_bp.get("/api/config/users/<user_id>")
def api_config_users_get_one(user_id):
    """Get a single monitored user configuration by user_id.

    Proxies to Sentinel API and handles ID normalization for matching.
    """
    import requests
    ...
    try:
        ...
        # Proxy to Sentinel API (single source of truth per DB_Architecture.instructions.md)
        sentinel_api_url = os.getenv(
            "SENTINEL_API_BASE_URL", "http://sentinel:8080/api"
        )
        response = requests.get(f"{sentinel_api_url}/config", timeout=5)
        ...
        # Try all possible ID variants (same normalization as channels)
        id_variants = _normalize_chat_id(user_id_int)
        for user in users:
            if user.get("id") in id_variants:
                return jsonify({"status": "ok", "user": user}), 200

        return jsonify({"status": "error", "message": "User not found"}), 404
    except requests.RequestException as e:
        logger.error(f"Request to Sentinel failed: {e}", exc_info=True)
        return jsonify({"status": "error", "message": "Sentinel unavailable"}), 502
    except Exception as e:
...
def _normalize_chat_id(chat_id_int: int) -> list:
    """Generate possible ID variants for matching.

    Telegram uses different ID formats:
    - Config may store: 1443765280 (positive, no prefix)
    - Full chat_id may be: -1001443765280 (negative with -100 prefix for channels/supergroups)

    This returns all possible variants to try matching against.
    """
    variants = [chat_id_int]
    abs_id = abs(chat_id_int)
    variants.append(abs_id)

    # If it starts with 100, also try without the prefix
    abs_str = str(abs_id)
    if abs_str.startswith("100") and len(abs_str) > 3:
        without_prefix = int(abs_str[3:])
        variants.append(without_prefix)

    # If it doesn't start with 100, also try with the prefix
    if not abs_str.startswith("100"):
        with_prefix = int("100" + abs_str)
        variants.append(with_prefix)

    return list(set(variants))


@config_info_bp.get("/api/config/channels/<chat_id>")
def api_config_channels_get_one(chat_id):
    """Get a single channel configuration by chat_id.

    Proxies to Sentinel API and handles ID normalization for matching.
    Handles both full Telegram chat_id (e.g., -1001443765280) and
    config-stored ID (e.g., 1443765280) by normalizing the lookup.
    """
    import requests
    ...
    try:
        ...
        # Try all possible ID variants
        id_variants = _normalize_chat_id(chat_id_int)
        for channel in channels:
            channel_id = channel.get("id")
            if channel_id in id_variants:
                return jsonify({"status": "ok", "channel": channel}), 200
...
def api_config_channels_patch(chat_id):
    """Update channel properties (e.g., enabled state) via Sentinel API."""
    try:
        ...
        # Use ID normalization for matching
        id_variants = _normalize_chat_id(chat_id)

        # Update channel properties
        for channel in channels:
            if channel.get("id") in id_variants:
                channel_found = True
