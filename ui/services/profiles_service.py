...
class ProfileService:
    ...
    def get_interest_profile(self, profile_id: int) -> Dict[str, Any] | None:
        """Get a single interest profile by ID via Sentinel API.

        Args:
            profile_id: Profile ID (integer in 3000-3999 range).

        Returns:
            Interest profile dictionary or None if not found.
        """
        try:
            profile_id = int(profile_id)  # Ensure integer
            url = f"{self.sentinel_api_base_url}/profiles/interest/{profile_id}"
            response = requests.get(url, timeout=10)
            ...
            if data.get("status") == "ok":
                ...
                logger.error(
                    f"Failed to get interest profile {profile_id}: {data.get('message')}"
                )
                return None

        except requests.exceptions.RequestException as exc:
            logger.error(
                f"Error getting interest profile {profile_id} from Sentinel API: {exc}"
            )
            return None
        except Exception as exc:
            logger.error(
                f"Unexpected error getting interest profile {profile_id}: {exc}"
            )
            return None
