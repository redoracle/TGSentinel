...
async def cache_avatars_parallel(
    ...
    YIELD_EVERY = (
        3  # Yield to event loop every N avatars to allow message handlers to run
    )
    ...
    for entity_id, photo, display_name in entities_with_photos:
        # Yield to event loop periodically to allow message handlers to run
        # This prevents avatar caching from monopolizing the Telethon client
        if processed_count > 0 and processed_count % YIELD_EVERY == 0:
            ...
            async def perform_cache_refresh():
                ...
                try:
                                ...
                                json.dumps(
                                    {
                                        ...
                                        "percent": 95,
                                        "message": (
                                            f"✓ Loaded {len(channels_list)} channels, "
                                            f"{len(users_list)} users. Caching avatars..."
                                        ),
                    ...
                    if entities_with_photos:
                        logger.info(
                            f"[CACHE-REFRESHER] Starting parallel avatar caching for "
                            f"{len(entities_with_photos)} entities..."
                        )
                ...
                try:
                    ...
                    while (
                        ...
                        if (
                            ...
                            logger.info(
                                "[CACHE-REFRESHER] Generation change or logout detected "
                                "during wait, exiting refresh loop"
                            )
                            ...
                            if message and message.get("type") == "message":
                                try:
                                    ...
                                    if event_type == "session_authorized":
                                        logger.info(
                                            "[CACHE-REFRESHER] ⚡ session_authorized event received, "
                                            "restarting for new generation"
                                        )
                                        session_change_detected = True
                                        break
