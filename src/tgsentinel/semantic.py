...
def load_profile_embeddings(
    ...
    """Load and encode positive/negative samples for a semantic profile.
    ...
    log.info(
        "[SEMANTIC] Encoding profile %s: %d positive, %d negative samples",
        profile_id,
        len(positive_samples),
        len(negative_samples),
    )

    # Encode positive samples and build normalized centroid
    # Individual vectors are normalized, then we average and re-normalize
    # This ensures dot products produce true cosine similarity in [-1, 1]
    positive_encoded = _model.encode(positive_samples, normalize_embeddings=True)
    positive_vec = _build_normalized_centroid(np.asarray(positive_encoded))

    # Encode negative samples and build normalized centroid (optional)
    negative_vec = None
    if negative_samples:
        negative_encoded = _model.encode(negative_samples, normalize_embeddings=True)
        negative_vec = _build_normalized_centroid(np.asarray(negative_encoded))
