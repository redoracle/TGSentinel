...
def evaluate_interest_profiles(
    ...
    # DEBUG: Log entry point with initial state
    log.info(
        "[INTERESTS-EVALUATOR] === FUNCTION CALLED === "
        f"resolved_profile={'present' if resolved_profile else 'None'}, "
        f"matched_profile_ids={resolved_profile.matched_profile_ids if resolved_profile else 'N/A'}, "
        f"global_profiles_count={len(cfg.global_profiles) if cfg.global_profiles else 0}, "
    ...
    if (
        ...
        log.warning(
            "[INTERESTS-EVALUATOR] Early return: "
            f"resolved_profile={'present' if resolved_profile else 'None'}, "
            f"has_matched_ids={bool(resolved_profile.matched_profile_ids if resolved_profile else False)}, "
            f"has_global_profiles={bool(cfg.global_profiles)}",
            extra=extra,
        )
        return None
    ...
    for pid in resolved_profile.matched_profile_ids:
        ...
        if not profile_def:
            log.warning(
                "[INTERESTS-EVALUATOR] Profile %s not found in global_profiles",
                pid,
                extra=extra,
            )
            continue
        ...
        log.debug(
            "[INTERESTS-EVALUATOR] Checking profile %s: is_semantic=%s, has_positive_samples=%s",
            pid,
            is_semantic,
            bool(getattr(profile_def, "positive_samples", None)),
            extra=extra,
        )
        ...
        if not is_semantic:
            ...
            log.debug(
                "[INTERESTS-EVALUATOR] Skipping non-semantic profile %s",
                pid,
                extra=extra,
            )
            continue
        ...
        log.debug(
            "[INTERESTS-EVALUATOR] Calculating semantic score for profile %s",
            pid,
            extra=extra,
        )
    ...
    if not interest_profile_ids:
        log.info(
            "[INTERESTS-EVALUATOR] No semantic profiles matched thresholds (checked %d profiles)",
            len(resolved_profile.matched_profile_ids),
            extra=extra,
        )
        return None
    ...
    matched_scores = [semantic_scores.get(pid, 0.0) for pid in triggering_feed_profiles]
    max_semantic_score = max(matched_scores) if matched_scores else 0.0
