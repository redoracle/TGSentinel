...
class UnifiedDigestWorker:
    ...
    def _aggregate_delivery_config(
        ...
        if len(modes) == 0:
            mode = "dm"  # Default if no modes found
        elif len(modes) == 1:
            mode = modes.pop()
        else:
    ...
    async def _send_digest(
        ...
        log.info(
            ...
            f"with {len(messages)} messages in {len(chunks)} part(s) to {target}",
            extra={
                "schedule": schedule.value,
        ...
        try:
            for i, chunk in enumerate(chunks):
                ...
                await client.send_message(
                    target, part_header + chunk, link_preview=False
                )
            log.info(
                f"[UNIFIED-DIGEST] Sent {schedule.value} digest to {target}",
                extra={"schedule": schedule.value, "target": target},
            )
        except Exception as e:
            log.error(
                f"[UNIFIED-DIGEST] Failed to send {schedule.value} digest to {target}: {e}",
                exc_info=True,
                extra={"schedule": schedule.value, "target": target},
            )
    ...
    def _format_digest(
        ...
        for idx, msg in enumerate(messages, 1):
            ...
            if isinstance(msg.created_at, datetime):
                ...
                log.warning(
                    "[UNIFIED-DIGEST] Message has non-datetime created_at",
                    extra={
                        "chat_id": msg.chat_id,
                        "msg_id": msg.msg_id,
