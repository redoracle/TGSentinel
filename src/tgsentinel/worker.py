...
def _create_default_rule(rid: int, name: str) -> ChannelRule:
    ...
    Uses dataclass defaults for most fields; only sets id, name, and
    non-zero engagement thresholds for sensible default behavior.
    """
    rule = ChannelRule(id=rid)
    rule.name = name
    rule.reaction_threshold = 5
    rule.reply_threshold = 3
    return rule
...
async def process_stream_message(
    ...
    if alert_result.should_alert:
        ...
        # Build message link for channels (negative chat IDs)
        message_link = None
        if rid < 0:  # Channel/group
            clean_id = (
                str(abs(rid))[3:] if str(abs(rid)).startswith("100") else str(abs(rid))
            )
            message_link = f"https://t.me/c/{clean_id}/{msg_id}"

        # Get timestamp and reactions from payload
        timestamp = payload.get("timestamp")
        reactions = payload.get("reactions")

        # Check if sender is VIP
        is_vip = sender_id in vip if sender_id else False

        # Get profile name from the first matching alert profile
        profile_name = None
        if all_triggering_profiles and profile_resolver:
            first_profile_id = all_triggering_profiles[0]
            if first_profile_id in profile_resolver.global_profiles:
                profile_name = profile_resolver.global_profiles[first_profile_id].name

        # Fallback to resolved profile name if no specific match
        if not profile_name and resolved_profile:
            profile_name = resolved_profile.name

        # Convert triggers from comma-separated string to list for proper formatting
        triggers_list = None
        if triggers:
        ...
        profile_id = (
            resolved_profile.id
            if resolved_profile and hasattr(resolved_profile, "id")
            else None
        )

        # Pass semantic_score if message also matches interest criteria
        semantic_score_value = None
        if interest_result and interest_result.should_include_in_feed:
            semantic_score_value = interest_result.max_semantic_score
        ...
        delivery_payload = DeliveryPayload(
            ...
            semantic_score=semantic_score_value,  # Include if message also matches interests
            timestamp=timestamp,
            ...
            triggers=triggers_list,  # Pass as list for proper formatting
        )
    ...
    # Orchestrate delivery for interest-only messages (not already handled by alerts)
    if (
        interest_result
        and interest_result.should_include_in_feed
        and not alert_result.should_alert  # Only for pure interest messages
    ):
        # Get the first matching interest profile to check delivery settings
        interest_profile = None
        if interest_result.matched_profile_ids and profile_resolver:
            first_interest_profile_id = interest_result.matched_profile_ids[0]
            if first_interest_profile_id in profile_resolver.global_profiles:
                interest_profile = profile_resolver.global_profiles[
                    first_interest_profile_id
                ]

        # Check if interest profile has immediate delivery configured (schedule: none)
        should_deliver_immediately = False
        delivery_mode = None
        target = None

        if interest_profile and interest_profile.digest:
            for sched in interest_profile.digest.schedules:
                if sched.enabled and sched.schedule == DigestSchedule.NONE:
        ...
        if should_deliver_immediately and delivery_mode:
            delivery_mode = normalize_delivery_mode(delivery_mode) or "dm"

            # Get webhooks configuration from interest profile
            webhook_services = []
            if interest_profile and hasattr(interest_profile, "webhooks"):
                webhook_services = interest_profile.webhooks or []

            # Create delivery payload for interest message
            preview = str(payload["text"] or "").strip().replace("\n", " ")
            if len(preview) > 400:
                preview = preview[:400] + "…"
            ...
            # Get profile name from the first matching interest profile
            profile_name = None
            profile_id = None
            if interest_profile:
                profile_name = interest_profile.name
                profile_id = (
                    interest_profile.id if hasattr(interest_profile, "id") else None
                )

            # Interest messages don't have triggers (keyword-based)
            triggers_list = None

            interest_delivery_payload = DeliveryPayload(
                semantic_type="interest_semantic",
                delivery_mode=delivery_mode,
                delivery_target=target,
                message_text=preview,
                ...
                chat_id=rid,
                msg_id=msg_id,
                score=interest_result.max_semantic_score,
                matched_profiles=interest_result.matched_profile_ids,
                trigger_annotations={},  # No keyword triggers for interest-only
                keyword_score=None,  # Interest messages don't have keyword scores
                semantic_score=interest_result.max_semantic_score,
                timestamp=timestamp,
                message_link=message_link,
                reactions=reactions,
                is_vip=is_vip,
            ...
            # Create NotifierAdapter for interest delivery
            class InterestNotifierAdapter:
                """Adapter to use notifier functions with orchestrate_delivery."""

                def __init__(self, notifier_module):
                    self.notifier_module = notifier_module
                ...
                async def notify_dm(
                    ...
                    **kwargs,
                    ...
                    await self.notifier_module.notify_dm(
                        client=client,
                        title=title,
                        text=text,
                        target=target,
                ...
                async def save_to_telegram(
                    ...
                    **kwargs,
                    ...
                    await self.notifier_module.save_to_telegram(
                        client=client,
                        title=title,
                        text=text,
                        chat_title=chat_title,
            ...
            # Import the notifier module for interest delivery
            from . import notifier as notifier_module

            interest_notifier = InterestNotifierAdapter(notifier_module)

            try:
                interest_delivery_result = await orchestrate_delivery(
                    payload=interest_delivery_payload,
                    client=client,
                    notifier=interest_notifier,
                    webhooks_cfg=webhook_services,
                )

                log.info(
                    f"[WORKER] Interest delivery orchestrated for chat={rid}, msg={msg_id}: "
                    f"mode={interest_delivery_result.get('delivery_mode_used')}, "
                    f"profile={profile_name}, semantic_score={interest_result.max_semantic_score:.2f}"
                )
            except Exception as interest_delivery_exc:
                log.error(
                    f"[WORKER] Interest delivery orchestration failed for "
                    f"chat={rid}, msg={msg_id}: {interest_delivery_exc}",
                    exc_info=True,
                )
