...
class ResolvedProfile:
    ...
    detect_mentions: bool = False
    detect_questions: bool = False

    # Engagement thresholds
    reaction_threshold: int = 0  # Minimum reactions to boost score
    reply_threshold: int = 0  # Minimum replies to boost score
    ...
    id: Optional[str] = None  # Primary profile id (first bound)
    name: str = ""  # Primary profile name (first bound)
...
class ProfileResolver:
        ...
        for profile_id, profile in self.global_profiles.items():
            ...
            if profile_id not in effective_profiles:
                ...
                if both_empty or channel_matches:
                    ...
                    log.info(
                        f"[PROFILE-RESOLVER] Auto-binding global profile '{profile_id}' to channel {channel.id} "
                        f"({reason}) - channels={profile.channels}, users={profile.users}"
                    )
                # Removed log for non-matching profiles to reduce N×M log spam
        ...
        for profile_id, profile in self.global_profiles.items():
            ...
            if profile_id not in effective_profiles:
                ...
                if both_empty or user_matches:
                    ...
                    log.info(
                        f"[PROFILE-RESOLVER] Auto-binding global profile '{profile_id}' to user {user.id} "
                        f"({reason}) - channels={profile.channels}, users={profile.users}"
                    )
                # Removed log for non-matching profiles to reduce log spam and avoid exposing user IDs
    ...
    def _resolve(
        ...
        # Engagement thresholds (use max across profiles)
        ...
        for profile_id in bound_profiles:
            ...
            if not getattr(profile, "enabled", True):
                log.debug(
                    "Skipping disabled profile '%s' for %s %s",
                    profile_id,
                    entity_type,
                    entity_id,
                )
            ...
            if getattr(profile, "tags", None):
                merged_tags.update(getattr(profile, "tags", []))
            ...
            for flag_key in detect_flags.keys():
                detect_flags[flag_key].append(bool(getattr(profile, flag_key, False)))

            # Collect engagement thresholds (will use max)
            for threshold_key in engagement_thresholds.keys():
                engagement_thresholds[threshold_key].append(
                    int(getattr(profile, threshold_key, 0))
                )
            ...
            webhook_services.update(getattr(profile, "webhooks", []) or [])

            # Track primary profile metadata for downstream logging/notifications
            if primary_profile_id is None:
                primary_profile_id = getattr(profile, "id", profile_id)
                primary_profile_name = getattr(profile, "name", "") or str(profile_id)
        ...
        # Step 5: Set detection flags (use most permissive)
        ...
        resolved.prioritize_private = any(detect_flags["prioritize_private"])
        resolved.detect_polls = any(detect_flags["detect_polls"])
        resolved.detect_mentions = any(detect_flags["detect_mentions"])
        resolved.detect_questions = any(detect_flags["detect_questions"])

        # Step 5b: Set engagement thresholds (use max across profiles)
        resolved.reaction_threshold = (
            max(engagement_thresholds["reaction_threshold"])
            if engagement_thresholds["reaction_threshold"]
            else 0
        )
        resolved.reply_threshold = (
            max(engagement_thresholds["reply_threshold"])
            if engagement_thresholds["reply_threshold"]
            else 0
        )

        # Step 6: Resolve digest configuration
        ...
        resolved.webhooks = sorted(webhook_services)
        resolved.id = primary_profile_id
        resolved.name = primary_profile_name
