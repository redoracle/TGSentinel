...
- Copy `.env.sample` to `.env` and set:
  ...
  - `UI_SECRET_KEY`
  - Optional: `NOTIFICATION_MODE` (`dm|channel|both`), `NOTIFICATION_CHANNEL`, `HOURLY_DIGEST`, `DAILY_DIGEST`, `DIGEST_TOP_N`
...
- To start, keep `NOTIFICATION_MODE=dm` and leave `NOTIFICATION_CHANNEL` empty.
...
- **No alerts arriving:**
  ...
  - If using `channel`/`both`, set a valid `NOTIFICATION_CHANNEL`.
...
make test              # Recommended: runs all tests
