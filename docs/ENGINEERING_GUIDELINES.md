...
- **Environment variables** (deployment‑specific; secrets). Examples:
  ...
  - Alerts/Digests: `NOTIFICATION_MODE` (`dm|channel|both`), `NOTIFICATION_CHANNEL`, `HOURLY_DIGEST`, `DAILY_DIGEST`, `DIGEST_TOP_N`.
...
### Redis-Only State Persistence

The following state is persisted exclusively to Redis (no file-based fallback):

- **Digest schedule timestamps**: `tgsentinel:digest:last_run:{schedule}` — Last execution time per schedule
- **Digest execution history**: `tgsentinel:digest:executions:{profile_id}` — Execution records per profile
- **Batch processor state**: `tgsentinel:batch_processor:queue`, `tgsentinel:batch_processor:last_batch_time`

This simplifies recovery and eliminates file I/O race conditions.
...
The message formatting system is built on three components:

**FormatRegistry** (`message_formats/registry.py`):

- Centralized metadata for all format types (dm_alerts, saved_messages, digest_entry, etc.)
- Auto-discovers variables from templates via regex
- Validates sample data coverage
- Single source of truth for template definitions

**FormatterContext** (`message_formats/context.py`):

- Unified context builder for preview API and renderer
- Fluent interface: `.from_payload()`, `.from_sample()`, `.build()`
- Handles trigger formatting, message truncation, and line generation
- Eliminates duplicate context-building logic

**Line Builder with Diagnostics** (`message_formats/line_builder.py`):

- Generates pre-formatted line variables (`sender_line`, `vip_line`, etc.)
- Debug mode returns diagnostics explaining why lines were/weren't rendered
- Supports conditional rendering via `{?variable}` syntax

### Digest Execution Tracking

Digest runs are tracked via `DigestExecutionRecord` (`digest_execution.py`):

- **Status machine**: PENDING → RUNNING → SUCCESS | PARTIAL | FAILED | CANCELLED
- **Persisted to Redis**: `tgsentinel:digest:executions:{profile_id}` (list with TTL)
- **Queryable via API**: `GET /api/digest/executions` returns history with stats
- **Metrics**: message count, duration, error messages, delivery mode
...
# Install development tools (for testing, linting, formatting)
pip install black isort flake8 mypy pytest pytest-cov pytest-asyncio ruff
```

**Critical**: All local Python commands (tests, formatting, linting, tools) **must** run inside the activated `.venv` virtual environment. The Makefile handles activation automatically for all targets.
...
- **UI locally** (requires activated venv):
...
Pytest suite under `tests/` covers config precedence, client ingestion, worker loop, digest, UI endpoints, and participant info flow.

**Local Development** (requires virtual environment):
...
- **No alerts**: ensure `NOTIFICATION_MODE` and (if needed) `NOTIFICATION_CHANNEL`; verify channel IDs in YAML; inspect worker logs.
