...
1. Check environment variable NOTIFICATION_MODE
...
NOTIFICATION_MODE=both                       # Options: dm | channel | both
NOTIFICATION_CHANNEL=@your_bot_name          # Target channel/bot username
...
SIMILARITY_THRESHOLD=0.55             # Recommended threshold (code default: 0.42)
```
...
alerts:
  mode: "dm" # Overridden by NOTIFICATION_MODE env var
  target_channel: "" # Overridden by NOTIFICATION_CHANNEL env var
  digest:
...
1. **Check alert mode:**
   ...
   ```bash
   docker compose exec sentinel env | grep NOTIFICATION_
