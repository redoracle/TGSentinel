...
If you plan to run tests or tools locally (outside Docker):
...
**Important**: All local Python commands (tests, tools, scripts) must run inside the activated virtual environment. The Makefile handles this automatically.
...
- `NOTIFICATION_MODE`: Where to send alerts (`dm`, `channel`, or `both`)
- `NOTIFICATION_CHANNEL`: Target channel/bot username (if using `channel` or `both`)
...
make test              # Run all tests via tools/run_tests.py
...
- Check `NOTIFICATION_MODE` env var: `dm`, `channel`, or `both`
- If using `channel` mode, ensure `NOTIFICATION_CHANNEL` is set to valid channel/bot username
