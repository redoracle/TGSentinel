...
def test_alerts_view(app_client):
    """Test alerts page renders or redirects to auth."""
    response = app_client.get("/alerts")
    # May redirect to auth if authentication is required
...
def test_api_session_info(app_client):
    """Test /api/session/info endpoint with proper Redis session data mocking."""
    import json

    import ui.app as flask_app

    # Mock Redis session data that would be populated by authenticated session
    mock_worker_status = json.dumps({"authorized": True, "status": "authorized"})

    mock_user_info = json.dumps(
        {
            "username": "test_user",
            "first_name": "Test",
            "phone": "+1234567890",
            "avatar": "/api/avatars/user/123456789",
        }
    )

    # Configure the mock redis_client that's already in the fixture
    def redis_get_side_effect(key):
        if key == "tgsentinel:worker_status":
            return mock_worker_status.encode()
        elif key == "tgsentinel:user_info":
            return mock_user_info.encode()
        return None

    flask_app.redis_client.get.side_effect = redis_get_side_effect
    ...
    # Phone should be masked: +1234567890 -> +*****90 (shows last 2 digits only)
    assert data["phone_masked"] == "+*****90"
...
def test_api_session_info_unauthorized(app_client):
    """Test /api/session/info endpoint when not authorized."""
    import ui.app as flask_app

    # Configure mock Redis to show unauthorized state (no session data)
    flask_app.redis_client.get.return_value = None
    flask_app.redis_client.get.side_effect = None  # Clear any side effects
    ...
    assert "username" in data
    assert "phone_masked" in data
    assert data["phone_masked"] == "Not linked"  # Shows "Not linked" when no phone
    assert "connected" in data
    assert "authorized" in data
