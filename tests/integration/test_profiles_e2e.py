...
def test_profile_resolution_e2e(temp_profiles_config):
    ...
    # Trigger annotations are validated above - formatting is now handled
    # by the message_formats rendering system, not a standalone function
...
def test_backward_compatibility():
    """Test that legacy keyword fields no longer work without profiles (removed in refactor)."""
    ...
    # Verify empty lists - legacy keywords are ignored without profiles
    assert "urgent" not in resolved.keywords
    assert "CVE" not in resolved.security_keywords
