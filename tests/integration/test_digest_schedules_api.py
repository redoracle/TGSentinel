...
def test_profile_digest_config_endpoint_returns_profile_details(
    ...
    schedules_by_type = {entry["schedule"]: entry for entry in data["schedules"]}
    hourly_schedule = schedules_by_type["hourly"]
    assert hourly_schedule["top_n"] == 6
    assert hourly_schedule["min_score"] == 4.2
    assert hourly_schedule["mode"] == "both"
    assert hourly_schedule["target_channel"] == "-100200"

    daily_schedule = schedules_by_type["daily"]
    assert daily_schedule["daily_hour"] == 9
    assert daily_schedule["mode"] == "both"
    assert daily_schedule["target_channel"] == "-100200"
