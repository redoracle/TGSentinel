...
def test_db():
    ...
    cursor.executemany(
        ...
        "(chat_id, msg_id, content_hash, score, flagged_for_alerts_feed, created_at) "
        "VALUES (?, ?, ?, ?, ?, ?)",
        test_data,
    )
    conn.commit()
...
def app_with_test_data(test_db, test_redis):
    ...
    mock_config.channels = [
        ...
        SimpleNamespace(id=742138586, name="Redoracle Security", enabled=True),
    ]
...
def test_dashboard_activity_with_empty_redis(test_db, test_redis):
    ...
    with patch("ui.app.load_config", return_value=mock_config):
        with patch("redis.Redis") as mock_redis_class:
            ...
            import ui.app as flask_app  # type: ignore[import-not-found]
            ...
            entries = data["entries"]
            assert isinstance(entries, list)
            # When Redis is empty, entries list should still be present (even if non-empty in current behavior)
