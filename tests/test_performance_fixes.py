...
def test_config_save_accepts_dm_mode():
    ...
    from app import _validate_config_payload
...
def test_config_save_rejects_invalid_mode():
    ...
    # Test old invalid values
    with pytest.raises(ValueError, match="Invalid mode"):
        _validate_config_payload({"mode": "direct"})

    with pytest.raises(ValueError, match="Invalid mode"):
        _validate_config_payload({"mode": "digest"})

    with pytest.raises(ValueError, match="Invalid mode"):
        _validate_config_payload({"mode": "invalid"})
...
def test_sentinel_restart_docker_compose_with_hyphen(
    ...
    # Mock Redis to return None (optional dependency)
    mock_redis_module = None

    mock_run.return_value = Mock(returncode=0, stdout="Restarted", stderr="")

    with app.test_client() as client:
        # Initialize app state with mocked dependencies
        from app import init_app, _is_initialized
        ...
        response = client.post("/api/sentinel/restart")

        # Should call docker-compose (with hyphen)
        mock_run.assert_called_once()
        call_args = mock_run.call_args[0][0]
        assert call_args[0] == "docker-compose"
        assert "restart" in call_args
        assert "sentinel" in call_args
...
def test_sentinel_restart_timeout_handling(mock_run, mock_redis_module, mock_init_db):
    ...
    mock_run.side_effect = TimeoutExpired("docker-compose", 30)
    ...
    with app.test_client() as client:
        ...
        assert response.status_code == 500  # Returns 500, not 504
