...
def test_profile_resolver_backward_compat():
    """Test backward compatibility with legacy keyword fields."""
    ...
    channel = ChannelRule(
        ...
        name="Test",
        vip_senders=[],
        keywords=["legacy1", "legacy2"],  # Old-style keywords
        security_keywords=["CVE"],
    ...
    assert "legacy1" in resolved.keywords
    assert "legacy2" in resolved.keywords
    assert "CVE" in resolved.security_keywords
...
def test_validate_profiles_duplicate():
    ...
    global_profiles = {
        "security": ProfileDefinition(id="security", name="Security"),
        "security": ProfileDefinition(id="security", name="Duplicate"),  # Duplicate!
    }
