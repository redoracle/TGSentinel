...
def test_env_vars_override_yaml_alerts(temp_config_file):
    """Test that NOTIFICATION_MODE and NOTIFICATION_CHANNEL env vars override YAML."""
    os.environ["TG_API_ID"] = "12345"
    os.environ["TG_API_HASH"] = "test_hash"
    os.environ["NOTIFICATION_MODE"] = "both"
    os.environ["NOTIFICATION_CHANNEL"] = "@test_bot"
...
def test_env_vars_override_yaml_redis(temp_config_file):
    ...
    try:
        config = load_config(temp_config_file)
        assert config.system.redis.host == "custom-redis"
        assert config.system.redis.port == 6380
        assert config.system.redis.stream == "custom:stream"
    finally:
        for key in [
            "TG_API_ID",
            "TG_API_HASH",
...
def test_yaml_defaults_when_no_env_vars(temp_config_file):
    ...
    for key in ["NOTIFICATION_MODE", "NOTIFICATION_CHANNEL", "HOURLY_DIGEST"]:
        os.environ.pop(key, None)
...
def test_config_instance_type(temp_config_file):
    ...
    try:
        ...
        assert hasattr(config.system, "redis")
        assert hasattr(config.system, "database_uri")
    finally:
        os.environ.pop("TG_API_ID", None)
        os.environ.pop("TG_API_HASH", None)
