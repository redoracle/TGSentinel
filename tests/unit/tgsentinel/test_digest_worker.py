...
class TestFormatDigest:
    ...
    def test_basic_digest_format(self, digest_worker):
        ...
        # Check header contains digest type and message count
        assert "Hourly Digest" in result
        assert "Top 1" in result or "1 messages" in result
        assert "1 channels" in result or "channel" in result

        # Check message content
        assert "Security Channel" in result
        assert "8.5" in result or "8.50" in result  # Score formatting
    ...
    def test_multiple_messages(self, digest_worker):
        ...
        # Check numbering - format is: "1. 💬 **[Channel Title](link)** (Score: X)"
        assert "1." in result
        assert "2." in result
    ...
    def test_message_text_truncation(self, digest_worker):
        ...
        # Current template uses {message_text} which shows full text, not truncated
        # The renderer provides both message_text (full) and message_preview (truncated)
        # The test validates that the full text IS present
...
class TestDeliveryModeValidation:
    ...
    # NOTE: test_channel_mode_without_target_falls_back_to_dm removed
    # 'channel' mode was deprecated and removed as part of legacy code cleanup.
    # Delivery is now determined by DeliveryMode enum (NONE, DM, DIGEST, BOTH).
