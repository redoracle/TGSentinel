...
class DummyRedisManager:
    def __init__(self, schedule_times=None):
        self.store = schedule_times or {}

    def get_all_digest_schedule_times(self):
        return dict(self.store)

    def set_digest_schedule_time(self, schedule, timestamp, ttl=0):
        self.store[schedule] = timestamp
...
class TestRedisStateIntegration:
    """Ensure Redis-backed timestamps are integrated with the scheduler."""

    def test_loads_state_from_redis(self):
        cfg = create_test_app_cfg()
        timestamp = datetime(2025, 11, 20, 8, 15, tzinfo=timezone.utc)
        redis_mgr = DummyRedisManager({"hourly": timestamp.isoformat()})

        scheduler = DigestScheduler(cfg, redis_manager=redis_mgr)

        assert scheduler.get_schedule_last_run(DigestSchedule.HOURLY) == timestamp

    def test_mark_schedule_run_writes_back_to_redis(self):
        cfg = create_test_app_cfg()
        redis_mgr = DummyRedisManager()
        scheduler = DigestScheduler(cfg, redis_manager=redis_mgr)

        timestamp = datetime(2025, 11, 20, 9, 0, tzinfo=timezone.utc)
        scheduler.mark_schedule_run(DigestSchedule.HOURLY, timestamp)

        assert redis_mgr.store["hourly"] == timestamp.isoformat()
