            ...
            def get_dynamic_sentinel_response(*args, **kwargs):
                ...
                mock_response.json.return_value = {
                    ...
                    "data": {
                        ...
                        "alerts": {
                            "mode": os.getenv("NOTIFICATION_MODE", "dm"),
                            "target_channel": os.getenv("NOTIFICATION_CHANNEL", ""),
                        },
...
def test_api_config_current_returns_alerts_config(app_client):
    ...
    os.environ["NOTIFICATION_CHANNEL"] = "@your_notification_bot"
    ...
    assert data["alerts"]["target_channel"] == "@your_notification_bot"

    # Cleanup
    del os.environ["NOTIFICATION_MODE"]
    del os.environ["NOTIFICATION_CHANNEL"]
...
def test_config_page_renders(app_client):
    ...
    assert b"embedding-model" in response.data
    assert b"decay-window" in response.data
    assert b"redis-host" in response.data
