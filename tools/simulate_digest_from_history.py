...
sends a digest to the destination configured in .env (NOTIFICATION_MODE/NOTIFICATION_CHANNEL).
...
from tgsentinel.store import init_db, mark_for_alerts_feed, upsert_message
...
def _score_and_store(cfg: AppCfg, payload: dict, engine) -> bool:
    ...
    score = hr.pre_score
    # Note: Semantic scoring removed - use profile-based approach via worker
...
async def main() -> int:
    ...
    stream_name = args.stream or cfg.system.redis.stream

    # Fix database path for local execution (Docker uses /app/data, local uses ./data)
    db_uri = cfg.system.database_uri
    ...
    if not args.no_process:
        log.info(
            "Connecting to Redis at %s:%s", cfg.system.redis.host, cfg.system.redis.port
        )
        r = Redis(
