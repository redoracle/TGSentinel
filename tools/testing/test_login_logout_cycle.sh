...
# Automatically discover available session files
echo "Discovering available session files..."
SESSIONS=()
shopt -s nullglob  # Handle case when no .session files exist

# Search for .session files in current directory and common locations
for pattern in "*.session" "../*.session" "./*.session"; do
...
if [ ${#SESSIONS[@]} -eq 0 ]; then
    echo -e "${RED}✗ No .session files found in current directory or parent directory${NC}" >&2
    echo "  Please ensure at least one .session file is present" >&2
    exit 1
fi

echo -e "${GREEN}✓ Found ${#SESSIONS[@]} session file(s):${NC}"
...
test_session() {
    ...
    # Verify session file exists before attempting upload
    if [ ! -f "$session_file" ]; then
        echo -e "${RED}✗ Session file not found: $session_file${NC}" >&2
    ...
    # Defensive check: handle Redis "(nil)" or empty response
    if [ -z "$worker_status" ] || [ "$worker_status" = "(nil)" ]; then
        echo -e "${RED}✗ worker_status is empty or (nil)${NC}"
        echo "   Raw value: '$worker_status'"
        return 1
    fi
    
    # Parse authorized with null-coalescing fallback and suppress jq stderr
    authorized=$(echo "$worker_status" | jq -r '.authorized // empty' 2>/dev/null)
    
    # Check if parsing failed (empty result)
    if [ -z "$authorized" ]; then
        echo -e "${RED}✗ Failed to parse worker_status JSON${NC}"
        echo "   Raw worker_status: $worker_status"
        return 1
    fi
    
    if [ "$authorized" = "true" ]; then
        echo -e "${GREEN}✓ Worker authorized${NC}"
        echo "   Status: $(echo "$worker_status" | jq -r '.status // "unknown"' 2>/dev/null)"
    else
        echo -e "${RED}✗ Worker not authorized${NC}"
        echo "$worker_status" | jq . 2>/dev/null || echo "$worker_status"
        return 1
    fi
    ...
    # Defensive check: handle Redis "(nil)" or empty response
    if [ -z "$worker_status_after" ] || [ "$worker_status_after" = "(nil)" ]; then
        echo -e "${RED}✗ worker_status_after is empty or (nil)${NC}"
        echo "   Raw value: '$worker_status_after'"
        return 1
    fi
    
    # Parse with null-coalescing fallback and suppress jq stderr
    authorized_after=$(echo "$worker_status_after" | jq -r '.authorized // empty' 2>/dev/null)
    status_after=$(echo "$worker_status_after" | jq -r '.status // empty' 2>/dev/null)
    
    # Check if parsing failed (empty result)
    if [ -z "$authorized_after" ] || [ -z "$status_after" ]; then
        echo -e "${RED}✗ Failed to parse worker_status_after JSON${NC}"
        echo "   Raw worker_status_after: $worker_status_after"
        return 1
    fi
    ...
    if [ "$authorized_after" = "false" ] && [ "$status_after" = "logged_out" ]; then
        ...
        echo "$worker_status_after" | jq . 2>/dev/null || echo "$worker_status_after"
        return 1
    fi
