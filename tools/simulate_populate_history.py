...
# 1. Stop the Sentinel container to avoid db locks
docker compose sentinel stop

# 2. Run the populate history script
docker compose run --rm sentinel python tools/simulate_populate_history.py \
  --limit 100 \
  --channel-id 726051464 \
  --dry-run

# 3. To actually populate (remove --dry-run)
docker compose run --rm sentinel python tools/simulate_populate_history.py \
  --limit 100 \
  --channel-id 726051464

# HELP
docker compose run --rm sentinel python tools/simulate_populate_history.py --help
"""
